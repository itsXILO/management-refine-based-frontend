...
const ClassesCreate = () => {
  ...
  useEffect(() => {
    ...
    const fetchSubjects = async () => {
      try {
        const response = await fetch(`${BACKEND_BASE_URL}/api/subjects?page=1&limit=100`);
        const payload = (await response.json()) as { data?: Subject[] };
        if (isMounted) {
    ...
    const fetchTeachers = async () => {
      try {
        const response = await fetch(
          `${BACKEND_BASE_URL}/api/users?page=1&limit=100&role=teacher`,
        );
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            {!teachersLoading && teachers.length === 0 ? (
                              <SelectItem value="__no_teachers" disabled>
                                No teachers found
                              </SelectItem>
                            ) : null}
                          </SelectContent>
